...
@click.command()
@click.argument('input_filepath1', type=click.Path(exists=False))
@click.argument('models_dirpath', type=click.Path(dir_okay=True, exists=False))
@click.argument('report_output_dirpath', type=click.Path(dir_okay=True, exists=False))
def main(input_filepath1, models_dirpath, report_output_dirpath):
    """
    Create feature importance visualizations for a processed train dataset and a set of models.

    Parameters:
        input_filepath1 (str): The path of the processed train dataset in csr format
    ...
    # df_train_final = pd.read_csv('./data/processed/train_processed_final.csv')

    dillfile = open(input_filepath1, 'rb')
    X_pipeline = dill.load(dillfile)
    dillfile.close()

    dillfile2 = open('./data/interim/train_data_processing_pipeline2.dil', 'rb')  
    # data_processing_pipeline2 = dill.load(dillfile2)
    final_data_processing_pipeline = dill.load(dillfile2)
    # close file
    dillfile2.close()
    ...
    for tree_model in tree_models_list:
        ...
        # model_filepath = models_dirpath + 'XGB_roc_auc_rs_cv_model.sav'
        auc_model = pickle.load(open(model_filepath,'rb'))
        # xgb_auc_model = pickle.load(open(model_filepath,'rb'))
        ...
        # plt.barh(final_data_processing_pipeline[-1].get_feature_names_out()[sorted_idx1], xgb_auc_model.best_estimator_.named_steps["clf"].feature_importances_[sorted_idx1]);
        plt.xlabel(tree_model + " model Feature Importances");
        # plt.xlabel(tree_model + "XGB model Feature Importances");
        report_output_filepath = report_output_dirpath + tree_model + '_Model_Feature_Importances.png'
